<!---
Vivixell/Vivixell is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
### Hi there 👋 I'm Victor Robin Okon
A Solution Architect & DevOps Engineer building secure, scalable cloud-native infrastructure. I work across AWS, Kubernetes, and IaC — with a growing focus on observability and reliability engineering.

📜 **Certifications:** [View my badges on Credly](https://www.credly.com/users/ovr/badges/credly)

- AWS Certified Solutions Architect – Associate
- AWS Certified Developer – Associate
- AWS Certified Cloud Practitioner
- CCNA
---
### 🛠️ Tech Stack
| Cloud | IaC | Containers & Orchestration | CI/CD & GitOps | Observability | Security Tooling |
| --- | --- | --- | --- | --- | --- |
| AWS | Terraform | Docker | GitHub Actions | Prometheus | Gitleaks |
| Azure | CloudFormation | Kubernetes | ArgoCD | Grafana | SonarQube |
| GCP | Ansible |  |  |  | Trivy |
---
### Featured Projects ✨
- **[DevSecOps Pipeline for a Microservices App](https://github.com/Vivixell/microservices-demo)**: Production-grade DevSecOps deployment of Google's Online Boutique (11-service microservices e-commerce app) on AWS EKS. Modular Terraform, GitHub Actions CI/CD, GitOps via ArgoCD, security scanning pipeline, and full Prometheus/Grafana observability.

- **[Cross-Account CI/CD Blue/Green Pipeline](https://github.com/Vivixell/cross-account-ecs-bluegreen-pipeline)**: Cross-account CI/CD pipeline deploying a containerized app to ECS Fargate with CodeDeploy Blue/Green traffic shifting, built with CloudFormation.

- **[Multi-Account Guardrails](https://github.com/Vivixell/multi-account-guardrails)**: Centralized governance for an AWS Organization — SCP guardrails, delegated administration, org-wide Config compliance, and IAM Identity Center access, defined in CloudFormation.

---

### 🔗 Connect
[![LinkedIn](https://img.shields.io/badge/LinkedIn-connect-blue?logo=linkedin)](https://linkedin.com/in/victorrobino)

[![Credly](https://img.shields.io/badge/Credly-certifications-green?logo=credly)](https://www.credly.com/users/ovr/badges/credly)

Always open to conversations on cloud architecture, DevOps practice, and reliability engineering. Let's build something great 🚀
